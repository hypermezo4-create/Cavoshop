...
import { Plus, Link as LinkIcon, Edit2, Trash2, X, Loader2, Instagram, Send, Twitter, MessageCircle, Share2, Shield, Globe, Terminal, Activity } from "lucide-react";
        ...
        <div className="p-6 lg:p-10 space-y-10">
            ...
            <header className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                <div>
                    ...
                    <h1 className="text-3xl md:text-5xl font-black text-white tracking-tighter">Social Links</h1>
                    <p className="text-zinc-500 text-sm font-medium mt-1">Manage Cavo social media links and customer-facing channels.</p>
                </div>
                <button
                ...
                >
                    <Plus className="w-4 h-4 group-hover:rotate-90 transition-transform" /> Add Social Link
                </button>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-6">
                {loading ? (
                    ...
                    <div className="col-span-full py-20 text-center glass-premium rounded-[2.5rem]">
                        ...
                        <h3 className="text-lg font-bold text-zinc-500">No social profiles added</h3>
                        <p className="text-sm text-zinc-600">Add your social links to display them across the storefront.</p>
                    </div>
                ) : links.map((link, i) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className="group glass-premium p-8 rounded-[2.5rem] hover:border-amber-500/30 transition-all relative overflow-hidden"
                        >
                            ...
                            <div className="relative z-10 flex items-start gap-5 mb-8">
                                <div className="w-16 h-16 rounded-[1.5rem] bg-amber-600/10 border border-amber-600/20 flex items-center justify-center text-amber-400 shadow-xl shadow-black/50">
                        ...
                        >
                            <div className="flex justify-between items-start mb-10">
                                <div>
                                    <div className="text-[10px] font-black text-amber-500 uppercase tracking-[0.4em] mb-2 flex items-center gap-2">
                                        <Shield className="w-3 h-3" /> Social Link Manager
                                    </div>
                                    <h2 className="text-3xl font-black text-white tracking-tighter">{editingId ? "Edit Social Link" : "Add Social Link"}</h2>
                                </div>
                                <button
                            ...
                            <form onSubmit={handleSubmit} className="space-y-8">
                                <div className="space-y-3 group">
                                    <label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest opacity-80 group-focus-within:text-amber-400 group-focus-within:opacity-100 transition-all">Platform</label>
                                    <select
                                        required
                                        value={formData.platform}
                                        onChange={e => setFormData({ ...formData, platform: e.target.value })}
                                        className="w-full bg-white/[0.03] border border-white/[0.05] rounded-[1.25rem] px-5 py-4 text-white focus:outline-none focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500/30 transition-all font-black appearance-none"
                                    >
                                        <option value="" className="bg-[#030406] text-zinc-600">Select platform...</option>
                                        <option value="instagram" className="bg-[#030406] text-white">INSTAGRAM</option>
                                        <option value="whatsapp" className="bg-[#030406] text-white">WHATSAPP</option>
                                ...
                                <div className="space-y-3 group">
                                    <label className="text-[10px] font-black text-zinc-600 uppercase tracking-widest opacity-80 group-focus-within:text-amber-400 group-focus-within:opacity-100 transition-all">Profile URL</label>
                                    <input
                                        required
                                        type="url"
                                        value={formData.url}
                                ...
                                <div className="flex items-center gap-6 p-6 rounded-[1.5rem] bg-white/[0.03] border border-white/[0.05] group cursor-pointer" onClick={() => setFormData({ ...formData, isActive: !formData.isActive })}>
                                    <div className={cn(
                                        ...
                                        formData.isActive ? "bg-amber-600" : "bg-zinc-800"
                                    )}>
                                        <div className={cn(
                                    ...
                                    <div className="flex flex-col">
                                        <span className="text-white font-black text-sm uppercase tracking-tight">Active Status</span>
                                        <span className="text-[9px] font-bold text-zinc-500 uppercase tracking-widest">Show this link across the storefront</span>
                                    </div>
                                </div>
                                ...
                                >
                                    {isSubmitting ? (
                                        ...
                                        <>{editingId ? "Save Changes" : "Save Link"} <Terminal className="w-4 h-4" /></>
                                    )}
                                </button>
                            </form>
                        </motion.div>
